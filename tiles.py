...
class MapTile(mixins.DataFileMixin):
    ...
    def fill_room(self, character):
        if not self.room_filled:
            ...
            for article in self.room_data['clothing']:
                self.items.append(items.Clothing(item_name=article))
            for weapon in self.room_data['weapons']:
                self.items.append(items.Weapon(item_name=weapon))
            ...
            for npc in self.room_data['hidden']['npcs']:
                ...
                self.hidden[-1].start()
            for item in self.room_data['hidden']['items']:
                self.hidden.append(items.Miscellaneous(item_name=item))
